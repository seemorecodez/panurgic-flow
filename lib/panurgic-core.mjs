...
PATTERN: raw evidence -> local evidence packet -> Codex forge -> sealed capsule.`,
  ...
  technicalProof:
    "The public workflow normalizes supplied evidence, maps claims to sources, and seals a portable packet with SHA-256.",
  codexNotes:
  ...
  workflow:
    "Capture evidence, review grounded claims, export artifacts, and verify sealed records.",
});
...
export function buildBrowserPacket(form, options = {}) {
  ...
  const claims = [
    `${name} preserves supplied development evidence as a portable record.`,
    "Generated claims remain connected to the strongest available source.",
    "The public workflow processes project material in the browser.",
  ];
  ...
  const manifest = {
    thesis: `${name} turns AI-assisted development records into grounded artifacts and verifiable evidence.`,
    ...
    verificationChecklist: [
      ...
      "Seal the packet only after the artifacts are approved.",
    ],
  };
  const artifacts = {
...
7. Seal the approved packet and retain its SHA-256 fingerprint.
...
6. Seal the capsule and confirm the fingerprint status is Verified.
...
4. Confirm its source label, review its claims, and seal it after approval.

## Expected behavior
- No project evidence is uploaded by the hosted workflow.
- Invalid or oversized packets are rejected with a clear message.
- A modified sealed packet is labeled Modified rather than trusted.`,
    codexSkill: `---
name: panurgic-flow
description: Turn AI-assisted development evidence into grounded artifacts and a sealed, reusable workflow packet.
---
...
6. Seal and export the approved V1 packet.
