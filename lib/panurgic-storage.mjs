import { createSigningIdentity } from "./panurgic-contract.mjs";
...
const SIGNING_STORE = "signing-identities";
const DEVICE_SIGNING_ID = "device-signing-key-v1";
const memoryProjects = new Map();
const memoryVersions = new Map();
let memorySigningIdentity;
let databasePromise;
  ...
  if (!databasePromise) {
    databasePromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!database.objectStoreNames.contains(SIGNING_STORE)) {
          database.createObjectStore(SIGNING_STORE, { keyPath: "id" });
        }
      };
      request.onsuccess = () => resolve(request.result);
...
export async function clearProjects() {
  ...
  memorySigningIdentity = undefined;
  try {
    await withIndexedDb(async (database) => {
      const transaction = database.transaction(
        [PROJECT_STORE, VERSION_STORE, SIGNING_STORE],
        "readwrite",
      );
      transaction.objectStore(PROJECT_STORE).clear();
      transaction.objectStore(VERSION_STORE).clear();
      transaction.objectStore(SIGNING_STORE).clear();
      await transactionComplete(transaction);
    });
    return { mode: "indexeddb" };
  } catch {
...
export async function getOrCreateSigningIdentity() {
  try {
    const identity = await withIndexedDb(async (database) => {
      const readTransaction = database.transaction(SIGNING_STORE, "readonly");
      const existing = await requestResult(
        readTransaction.objectStore(SIGNING_STORE).get(DEVICE_SIGNING_ID),
      );
      await transactionComplete(readTransaction);
      if (existing?.privateKey && existing?.publicKeyJwk && existing?.fingerprint) {
        return existing;
      }
      const generated = await createSigningIdentity();
      const created = {
        id: DEVICE_SIGNING_ID,
        createdAt: new Date().toISOString(),
        ...generated,
      };
      const writeTransaction = database.transaction(SIGNING_STORE, "readwrite");
      writeTransaction.objectStore(SIGNING_STORE).put(created);
      await transactionComplete(writeTransaction);
      return created;
    });
    memorySigningIdentity = identity;
    return { mode: "indexeddb", identity };
  } catch {
    if (!memorySigningIdentity) {
      memorySigningIdentity = {
        id: DEVICE_SIGNING_ID,
        createdAt: new Date().toISOString(),
        ...(await createSigningIdentity()),
      };
    }
    return { mode: "memory", identity: memorySigningIdentity };
  }
}
